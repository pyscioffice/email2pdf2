...
def handle_args(argv):
    ...
    parser.add_argument('--handle-eml-attachments', action='store_true', default=False,
                        help="By default email2pdf2 will ignore eml as attachments to the original email. This option will handle them as proper attachments.")
...
def handle_attachments(input_email, output_directory, add_prefix_date, ignore_floating_attachments, parts_to_ignore):
    ...
    for part in parts:
        ...
        if filename.endswith('.eml'):
            ...
            with open(full_filename, 'w') as emlfile:
                gen = generator.Generator(emlfile)
                gen.flatten(part)
        else:
            decode = not part.is_multipart() and True or False
    ...
    for i, part in enumerate(parties):
        ...
        if part not in parts_to_ignore:
            if part.get_content_type() == "message/rfc822" and part.get_content_disposition():
                parts.add(part)
            elif not part.is_multipart() and part.get_content_type() not in MIME_TYPES_BLACKLIST:
                parts.add(part)
